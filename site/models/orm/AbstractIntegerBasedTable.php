...
abstract class AbstractIntegerBasedTable implements InterfaceSpreadsheetTable
{
     ...
     */
    protected function getCachedObject(int $id)
    {
    ...
    public function create(array $row) : AbstractTableRowObject
    {
        ...
        if (!isset($row['id']) || is_int($row['id']) === false)
        {
     ...
     * the original ID.
     * @param AbstractIntegerTableRowObject $object
     * @return void
     */
    public function updateCache(AbstractIntegerTableRowObject $object) : void
    {
     ...
     */
    public function load(int $id, $useCache=true) : AbstractIntegerTableRowObject
    {
     ...
     */
    public function update(int $id, array $row) : AbstractTableRowObject
    {
        $query =
     ...
     * @return void
     */
    public function batchSave(array $objects) : void
    {
        /* @var $object AbstractIntegerTableRowObject */
        $queries = [
        ...
        //die(print_r($multiQuery, true));
        $result = $this->getDb()->query($multiQuery);

        if ($result === false)
        {
            throw new Exception("Failed to batch save data to database.");
        }
    ...
    public function loadIdsNotIn(array $ids) : array
    {
        ...
        $result = $this->getDb()->query("SELECT * FROM {$this->getEscapedTableName()} WHERE id NOT IN ({$idsString})");
        return $this->convertPgResultToObjects($result);
    }
}
